...
def get_news_embeddings(news):
    ...
    if not(isinstance(news, str)):
        ...
        news_text = news
    # Transform text to vectors using a simple dictionary
    text_tokens = tokenizer.encode(news_text, return_tensors="pt")
    model_input = 512
    # Compute the embeddings using the dbert-ft model for style representation
    embeddings = [model_bert_ft(text_tokens[:, :min(model_input, text_tokens.shape[1])])[0][:, :, :].detach().numpy()]
    # Since the model only takes 512 tokens at a time, we split the text and get the mean of the final embeddings
...
def get_user_embeddings(historical_news, current_news, historical_embeddings=None):
    """
    Takes the historical news of a user (list of news) and a news to rank.
    Returns the embeddings using the dbert-ft model of both the historical news and the current news.
    In order to avoid computing embeddings of historical news multiple times, it is possible to give them as input
    """
...
def gmrf(y, alpha, beta):
    """
    Generalized Mean Rescaling Function
    Takes a vector of scores and returns a vector of scores having a different 
    distribution (alpha and beta define the new curvature)
    """
    if beta:
        if alpha < 0.5:
            y = y**(2*alpha) + 1
...
def reswhy(rankings, rank_as_scores, weights, alphas, betas):
    """
    Implementation of the Reswhy algorithm proposed by Julien Hay (2021)
    Takes as argument a list of rankings generated by different algorithms 
    Every ranking is a tuple of lists (ids, scores)
    Generates a new ranking by combining the different rankings.
    The hyperparameters must be given as arguments of the function.
    """
